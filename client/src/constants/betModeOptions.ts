...
/** 投注单位（厘/分/角/元），与玩法 betMode（单式/复式等）分离 */
...
/** 规范投注单位；旧版 betMode 字段若存的是单位也适用 */
export function normalizeBetUnitValue(raw: string): string {
  return normalizeBetModeValue(raw)
}

/** 从方案 config 读取投注单位（兼容旧版 betMode 误存） */
...
/** 将旧版存储值（如 "2" 表示 2元）规范为新数值字符串 */
export function normalizeBetModeValue(raw: string): string {
  const t = raw.trim()
  if (!t) return '2'
