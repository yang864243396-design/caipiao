...
> **本仓库（caipiao 根目录）**：`client/**`、`admin/**` 为 Vue3 前端，`backend/**` 为占位与说明，无 `project.mdc`；涉及 UI/设计时仍按下表与 `.agents/skills` 选用 skill 即可。
...
安装记录见 **`skills/skills-lock.json`**（与 `.agents/skills/` 目录名一致）。
