...
-- 上游 rules/v2 将二全中、二中特同置于「连码」；本地方案需要各自的类型、输入与结算语义。
...
-- 历史同步目录将 279–290 全部归到 g003；仅搬移两类各自的六个子玩法。
...
WITH targets(sub_id, type_id, type_label, sub_label) AS (
  VALUES
    ('279', 'erquanzhong', '二全中', '复式'),
    ('280', 'erquanzhong', '二全中', '拖头'),
    ('281', 'erquanzhong', '二全中', '生肖对碰'),
    ('282', 'erquanzhong', '二全中', '尾数对碰'),
    ('283', 'erquanzhong', '二全中', '生尾对碰'),
    ('284', 'erquanzhong', '二全中', '任意对碰'),
    ('285', 'erzhongte', '二中特', '复式'),
    ('286', 'erzhongte', '二中特', '拖头'),
    ('287', 'erzhongte', '二中特', '生肖对碰'),
    ('288', 'erzhongte', '二中特', '尾数对碰'),
    ('289', 'erzhongte', '二中特', '生尾对碰'),
    ('290', 'erzhongte', '二中特', '任意对碰')
)
...
SET config = jsonb_set(
    jsonb_set(
      jsonb_set(
        jsonb_set(
          jsonb_set(
            jsonb_set(d.config, '{playTypeId}', to_jsonb(t.type_id), true),
            '{typeId}', to_jsonb(t.type_id), true),
          '{playTypeLabel}', to_jsonb(t.type_label), true),
        '{playMethodLabel}', to_jsonb(t.sub_label), true),
      '{guajiGroup}', to_jsonb(t.type_label), true),
    '{guajiTeam}', to_jsonb(t.type_label), true)
FROM targets t
...
SET config = jsonb_set(
    jsonb_set(
      jsonb_set(
        jsonb_set(
          jsonb_set(
            jsonb_set(s.config, '{playTypeId}', to_jsonb(t.type_id), true),
            '{typeId}', to_jsonb(t.type_id), true),
          '{playTypeLabel}', to_jsonb(t.type_label), true),
        '{playMethodLabel}', to_jsonb(t.sub_label), true),
      '{guajiGroup}', to_jsonb(t.type_label), true),
